...
class fc(object):
    ...
    def forward(self, feat):
        ...
        output = np.dot(feat.reshape((-1, self.input_dim)), self.params[self.w_name]) + self.params[self.b_name]
        #############################################################################
        #                             END OF YOUR CODE                              #
        #############################################################################
        self.meta = feat
    ...
    def backward(self, dprev):
        ...
        # corresponding name.                                                       # 
        #############################################################################
        self.grads[self.w_name] = np.dot(np.transpose(feat.reshape(-1, self.input_dim)), dprev)
...
class relu(object):
    ...
    def backward(self, dprev):
        ...
        #############################################################################
        dfeat = np.where(feat>0, 1, 0) * dprev # Here is ele-wise
        #############################################################################
        #                             END OF YOUR CODE                              #
        #############################################################################
        self.meta = None
...
class cross_entropy(object):
    ...
    def forward(self, feat, label):
        ...
        #############################################################################
        # label one-hot to array
        ground_truth = np.zeros(feat.shape)
        ground_truth[np.arange(label.size), label] = 1
        loss = -np.sum(ground_truth * np.log(scores))
        if self.dim_average:
            loss = loss / label.size
    ...
    def backward(self):
        ...
        #############################################################################
        dLoss = dLoss - self.label
        if self.dim_average:
            dLoss = dLoss / self.label.shape[0]
        #############################################################################
        #                             END OF YOUR CODE                              #
        #############################################################################
        self.dLoss = dLoss
...
def softmax(feat):
    ...
    feat_sum = np.sum(feat_exp, axis = 1)
    # print(feat_exp.shape)
    # print(feat_sum.shape)
    scores = feat_exp / (feat_sum.reshape(feat.shape[0],-1))
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
    return scores