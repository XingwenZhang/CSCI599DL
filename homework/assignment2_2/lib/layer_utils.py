...
class LSTM(object):
	...
	def step_forward(self, x, prev_h, prev_c):
		...
		A = prev_h.dot(self.params[self.wh_name]) + x.dot(self.params[self.wx_name]) + self.params[self.b_name] # N * 4H
		ii, ff, oo, gg = np.hsplit(A, 4) # N * H
		i = sigmoid(ii)
		f = sigmoid(ff)
		o = sigmoid(oo)
		g = np.tanh(gg)
	...
	def forward(self, x, h0):
		...
		# c = h = np.zeros((x.shape[0],x.shape[1],h0.shape[1]))
		...
		c0 = np.zeros_like(h0)
		next_c = None
		h[:,0,:], next_c, meta_i = self.step_forward(x[:,0,:], h0, c0)
		self.meta.append(meta_i)
		for i in range(1, x.shape[1]):
			c0 = next_c
			h[:,i,:], next_c, meta_i = self.step_forward(x[:,i,:], h[:,i-1,:], c0)
			self.meta.append(meta_i)
		##############################################################################
		#							   END OF YOUR CODE							 	 #
		##############################################################################
	...
	def backward(self, dh):
		...
		T = dh.shape[1]
		H = dh.shape[2]
		D = self.meta[0][0].shape[1] # x.shape[1], x.shape = N,D
...
class word_embedding(object):
	...
	def forward(self, x):
		...
		# print('word_embedding out shape is {}'.format(out.shape))
		# print('word matrix shape is {}'.format(x.shape))
		self.meta = [out, x]
		##############################################################################
		#							   END OF THE CODE								 #
		##############################################################################
	...
	def backward(self, dout):
		...
		# print('w gradient shape is {}'.format(self.grads[self.w_name].shape))
		##############################################################################
		#							   END OF THE CODE								 #
		##############################################################################
